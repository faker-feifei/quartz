...
JS.Connector = JS.extend(JS.Observable, {
    ...
    startPostConnect: function() {
        if (this.running) {
            var url = this.url;
            this.param == utils.isEmpty(this.param) ? {} : this.param;
            this.param[this.CMDTAG] = "conn";
            this.param['cv'] = this.version;

            JS.AJAX.post(url, this.param,
                function(xhr) {
                    var msg = this.decodeMessage(xhr.responseText);
                    if (!msg) {
                        this.stop('连接失败');
